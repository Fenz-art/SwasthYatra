  ...
  return (
    <footer className="relative bg-canvas border-t border-hairline">
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(90deg, transparent, #00E5FF, #0066FF, transparent)",
        }}
      />
      ...
      <div className="max-w-[1240px] mx-auto px-6 pt-16 pb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8 mb-16">
          {footerColumns.map((column) => (
            <div key={column.title}>
              <h4 className="text-[14px] font-medium text-on-dark mb-4">{column.title}</h4>
              <ul className="space-y-2.5">
                {column.links.map((link) => (
                  <li key={link}>
                    <button className="text-[14px] text-mute hover:text-body transition-colors text-left">
                      {link}
                    </button>
                  </li>
                ))}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-8 border-t border-hairline">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[14px] font-medium text-on-dark">SwasthYatra</span>
          </div>
